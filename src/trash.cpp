void searchObjects(const std::unordered_map<int, Pipeline>& pls,
    ...
    std::cout << "Select search type (0 if by object type else 1 if by ID)."; bool search_type = GetCorrectNumber(0, 1);
    
    if (search_type == 0) {
        std::cout << "Select object type (0 if pipeline or 1 if compressor station). ";
        bool type = GetCorrectNumber(0, 1);
        
        if (type == 0) {
            std::cout << "Select the feature by which to search (0 if by name or 1 if by repairment status). ";
            bool feat = GetCorrectNumber(0, 1);
            ...
            if (feat == 0) {
                ...
                std::cout << "Enter repairment status (0 if without defects else 1). "; bool cur_rs = GetCorrectNumber(0, 1);
            ...
            if (feat == 0) {
                ...
                std::cout << "Enter uninvolved workshops percentage. "; int percentage = GetCorrectNumber(0, 100);
        ...
        std::cout << "Enter object ID. "; int search_id = GetCorrectNumber(0, 10000);
        ...
        switch (command) {
            ...
            case 2: {
                ...
                if (!filtered_pls.empty()) {
                    ...
                    bool new_status = GetCorrectNumber(0, 1);
                    ...
                    int new_iw = GetCorrectNumber(0, 100);
            ...
            case 3: {
				...
				// for (int id; id <= md.getID(); id++) {
				// 	if (!pls.count(id) && !css.count(id)) {
				// 		ids.push_back(id);
				// 	}
				// }

				// for (int id : ids) {
				// 	if (pls.count(id + 1) || css.count(id + 1)) {
...
int main() {
	...
	std::tm* time_now;
	time_now = getTime();
	file << time_now->tm_mday << "/" << time_now->tm_mon << "/" << time_now->tm_year + 1900 << " " << time_now->tm_hour << ":"
	<< time_now->tm_min << ":" << time_now->tm_sec << " : programme has been started" << std::endl;
    ...
    while (true) {
        ...
        switch (command) {
            case 0: {
				file << time_now->tm_mday << "/" << time_now->tm_mon << "/" << time_now->tm_year + 1900 << " " << time_now->tm_hour << ":"
				<< time_now->tm_min << ":" << time_now->tm_sec << " : programme has been ended" << std::endl << std::endl;
                return 0;
            }
            case 1: {
                Pipeline pl;
				...
				time_now = getTime();
				file << time_now->tm_mday << "/" << time_now->tm_mon << "/" << time_now->tm_year + 1900 << " " << time_now->tm_hour << ":"
				<< time_now->tm_min << ":" << time_now->tm_sec << " : pipeline with ID " << md.getID() << " created" << std::endl;
                continuing(); 
                break;
            }
            case 2: {
            ...
            case 5: {
				...
				file << time_now->tm_mday << "/" << time_now->tm_mon << "/" << time_now->tm_year + 1900 << " " << time_now->tm_hour << ":"
				<< time_now->tm_min << ":" << time_now->tm_sec << " : file " << filepath << " have been written" << std::endl;
